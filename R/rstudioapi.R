...
rs_replace_selection <- function(context, input) {
  ...
  # start streaming
  stream_selection(selection, context, assistant, n_lines_orig, selection_remainder, input = input)
}

# prefix selection with results -----------------------------------------------
rs_prefix_selection <- function(context, input) {
  assistant <- initialize_assistant(context = context, input = input)
  selection <- rstudioapi::primary_selection(context)
  ...
  stream_selection(selection, context, assistant, n_lines_orig = 1, input = input)
}

# suffix selection with results -----------------------------------------------
rs_suffix_selection <- function(context, input) {
  assistant <- initialize_assistant(context = context, input = input)
  selection <- rstudioapi::primary_selection(context)
...
# streaming --------------------------------------------------------------------
stream_selection <- function(selection, context, assistant, n_lines_orig, remainder = "", input) {
  tryCatch({
    if (is_positron()) {
      chat_selection_impl(
        selection = selection,
...
stream_selection_impl <- function(selection, context, assistant, n_lines_orig, remainder = "", input) {
  selection_text <- selection[["text"]]
  output_lines <- character(0)

  stream <- assistant$stream(construct_turn(input, context))
...
# so just chat rather than stream (simonpcouch/pal#68)
chat_selection_impl <- function(selection, context, assistant, remainder = "", input) {
  selection_text <- selection[["text"]]
  output_lines <- assistant$chat(construct_turn(input, context), echo = FALSE)
