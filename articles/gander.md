...
- As of early 2026, Anthropic’s **Claude Sonnet 4.6** is a very powerful
  model for code assistance and is the model I’ve used while developing
  the package. It’s fast, inexpensive, and strong at code tasks. If you
...
- Google’s **Gemini 3 Flash** is a fast, capable model. To use it,
  ...
  key](https://aistudio.google.com/apikey) to the environment variable
  `GOOGLE_API_KEY` and set:
  ...
  Setting `thinkingLevel = "minimal"` is important for latency—without
  it, the default dynamic thinking roughly doubles response time with no
  quality benefit for simple code transformation tasks.

- You can also use a **local model** with `chat_ollama()`, which uses
