...
Set the `.gander_style` option to some other string to tailor responses
...
`.gander_dims` allows you to adjust how many rows and columns to supply
to gander addin.
...
- To decrease token usage, decrease the number of rows and columns, e.g.
  `options(.gander_dims = c(0, 10))` to just show the names and types of
