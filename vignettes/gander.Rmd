...
gander uses the `.gander_chat` option to configure which model powers the addin. `.gander_chat` is an ellmer Chat object. For example, to use OpenAI's GPT-4o-mini, you might write `options(.gander_chat = ellmer::chat_claude())`. Paste that code in your `.Rprofile` via `usethis::edit_r_profile()` to always use the same model every time you start an R session.
